...
### 计费时点与失败恢复（本仓库实测补充，2026-09-24）

**计费发生在"创建任务"被接受的那一刻，不在下载成功时。** 任务一旦创建成功，即使随后
轮询超时、下载失败或任务 expired，**已预扣的费用不会自动退回**。由此产生三条硬性纪律
（`scripts/generate_video.py` 已实现）：

1. **创建请求绝不重试。** 响应丢失时无法判断任务是否已创建，重试可能**扣两次费**。
   创建超时单独用 `--create-timeout` 控制（默认 300s）——原写死 60s 会在 base64 首帧
   上传+排队较慢时，于任务已创建扣费之后误报失败。
2. **任务 ID 必须在轮询前落盘。** 脚本写入 `{mp4}.task.json`（原子写：先写临时文件再
   `os.replace`），记录 `task_id` / `base_url` / 请求体 / 状态。丢了这个 ID 就等于丢了
   已付费任务的唯一凭证。
3. **回捞不重新计费。** `--query <task_id>` 或 `--resume <task.json>` 只查询/下载，不创建。

**该网关没有取消接口。** 实测 `DELETE /…/tasks/{id}` 返回 `200 text/html`（2870 字节），
与一个确定不存在的路径 `/totally/not/a/route` 的响应**逐字节相同**——那是前端 SPA 兜底
页，不是 API。`PUT`/`PATCH` 同理。因此"取消任务"在此 provider 上无法实现。

### 查询路由的行为（重要，易误判）

`POST`（创建）与 `GET`（查询）的鉴权表现**不一致**，实测：

| 探测 | 结果 | 含义 |
|---|---|---|
| `POST` 有效模型 + 缺 `content` | `400 MissingParameter` | 鉴权通过、路由存活 |
| `POST` 无效模型 | `403 该令牌无权使用模型` | 鉴权通过 |
| `POST` 空体 | `503 SeeDance2 组无可用渠道` | 鉴权通过、组名正确 |
| `GET /tasks/{过期或不存在 id}` | `401 AuthenticationError`（**Volcengine 原生错误格式**） | 代理把"查不到"映射成 401 |

**结论：`GET` 返回 401 不等于令牌坏了或查询路由故障。** task_id 仅 **7 天**内可查
（见下文），对过期 ID 与凭空编造的 ID，`401` 的表现**完全一致**——无法据此区分"已过期"
与"路由故障"。要判定查询路由是否健康，**必须用一个 7 天内的新鲜 task_id**（即需要一次
真实付费提交），否则不构成证据。
